...
add_annual <- function(pairs_monthly, pairs_annual) {
  ...
  diff_df <- base::merge(pairs_annual, df, by = "reporter")
...
beta_filter <- function(df, reporters, partners, year, keyword) {
  ...
  if (!is.null(reporters)) { df <- base::merge(df, reporters_df, by = "reporter") }
  if (!is.null(partners)) { df <- base::merge(df, partners_df, by = "partner") }
