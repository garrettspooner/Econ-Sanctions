...
```{r}
#|: fifth-package-demo
#Package 5: yfR supports downloading time series stock market data from the Yahoo! finance site, straightforward library with seven functions supporting downloading data from Yahoo! finance

month_prior <- Sys.Date() - 30
today <- Sys.Date()
yf_get("TGT", first_date = month_prior, last_date = today, freq_data = "daily") #downloads stock market data for Target (TGT) with date and frequency specifications
```

```{r}
#|: sixth-package-demo

# Example: Fetching USD/RUB exchange rate data with alphavantager

av_api_key("AKODC2XAAZIFHH9J")
symbol <- "USD/RUB"
forex_data <- av_get(symbol, av_fun = "FX_DAILY", outputsize = "compact")

# The data is returned as an xts object
head(data)
```
