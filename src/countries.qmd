...
scrape_yahoo <- function(url, file, year) {
  scrape(url, file)
  df <- read_csv(
    file.path(
      dirname(getwd()),
...
# NEED TO CONCATENATE FIRST 3 COLUMNS, REMOVE FIRST 3 COLS AND MOVE CONCATENATED DATE COL TO FRONT, AND THEN RUN BELOW CODE
  ...
  # if (min_year > year) {
  #   print(paste0("Error: couldn't scrape data for ", year))
  # }
...
# old_scrape_yahoo <- function(url, file, year) {
#   num_obs <- ((year(Sys.Date()) - year) * 300)
...
```{r}
#|: global-var-to-scrape-same-years-of-data-for-all-countries

num_obs = 2010 # year for which to start gathering data
```
...
file = "china.csv"
china <- scrape_yahoo(url, file, num_obs)
```
...
file = "hong_kong.csv"
hong_kong <- scrape_yahoo(url, file, num_obs)
```
...
file = "india.csv"
india <- scrape_yahoo(url, file, num_obs)
```
...
file = "indonesia.csv"
indonesia <- scrape_yahoo(url, file, num_obs)
```
...
file = "israel.csv"
israel <- scrape_yahoo(url, file, num_obs)
```
...
file = "japan.csv"
japan <- scrape_yahoo(url, file, num_obs)
```
...
file = "malaysia.csv"
malaysia <- scrape_yahoo(url, file, num_obs)
```
...
philippines <- scrape_yahoo(url, file, num_obs)
```
...
saudi_arabia <- scrape_yahoo(url, file, num_obs)
```
...
file = "korea.csv"
korea <- scrape_yahoo(url, file, num_obs)
```
...
file = "taiwan.csv"
taiwan <- scrape_yahoo(url, file, num_obs)
```
...
file = "thailand.csv"
thailand <- scrape_yahoo(url, file, num_obs)
```
...
file = "bulgaria.csv"
bulgaria <- scrape_yahoo(url, file, num_obs)
```
...
file = "croatia.csv"
croatia <- scrape_yahoo(url, file, num_obs)
```
...
file = "england.csv"
england <- scrape_yahoo(url, file, num_obs)
```
...
file = "greece.csv"
greece <- scrape_yahoo(url, file, num_obs)
```
...
file = "ireland.csv"
ireland <- scrape_yahoo(url, file, num_obs)
```
...
file = "lithuania.csv"
lithuania <- scrape_yahoo(url, file, num_obs)
```
...
netherlands <- scrape_yahoo(url, file, num_obs)
```
...
file = "norway.csv"
norway <- scrape_yahoo(url, file, num_obs)
```
...
file = "portugal.csv"
portugal <- scrape_yahoo(url, file, num_obs)
```
...
file = "russia.csv"
russia <- scrape_yahoo(url, file, num_obs)
```
...
file = "serbia.csv"
serbia <- scrape_yahoo(url, file, num_obs)
```
...
file = "slovakia.csv"
slovakia <- scrape_yahoo(url, file, num_obs)
```
...
file = "spain.csv"
spain <- scrape_yahoo(url, file, num_obs)
```
...
file = "sweden.csv"
sweden <- scrape_yahoo(url, file, num_obs)
```
...
switzerland <- scrape_yahoo(url, file, num_obs)
```
...
file = "australia.csv"
australia <- scrape_yahoo(url, file, num_obs)
```
...
new_zealand <- scrape_yahoo(url, file, num_obs)
```
...
file = "argentina.csv"
argentina <- scrape_yahoo(url, file, num_obs)
```
...
file = "brazil.csv"
brazil <- scrape_yahoo(url, file, num_obs)
```
...
file = "canada.csv"
canada <- scrape_yahoo(url, file, num_obs)
```
...
file = "mexico.csv"
mexico <- scrape_yahoo(url, file, num_obs)
```
...
file = "usa.csv"
usa <- scrape_yahoo(url, file, num_obs)
```
